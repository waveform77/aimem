...
# Состав, размеры, координаты и порядок наборов оставлены как в исходной программе
JOBS = [
    {
        "name": "Блокноты",
        "subfolder": "Блокнот",
        "filename": "Блокноты.pdf",
        "page_size": (320, 450),
        "photo_size": (156, 218),
        "cols": 2,
        "rows": 2,
    ...
    {
        "name": "Наклейки Love is",
        "subfolder": "Наклейки Love is",
        "filename": "Наклейки Love is.pdf",
        "page_size": (320, 450),
        "photo_size": (49, 69),
        "cols": 6,
        "rows": 6,
...
def points_to_pixels(value_points: float, dpi: int = TARGET_IMAGE_DPI) -> int:
    """Размер PDF points -> пиксели целевого растра для вставки."""
...
def prepare_image_reader(
    ...
    """
    Готовит ImageReader без лишнего сохранения картинки в PNG через BytesIO.
    Для обычных ячеек дополнительно уменьшает исходник под реальный размер вставки.
    Координаты/размеры вставки в PDF не меняются.
    """
    if resize_to_target:
    ...
    key = (img_path, target_w_px, target_h_px, bool(resize_to_target))
    ...
    try:
        with Image.open(img_path) as img:
            if resize_to_target and target_w_px and target_h_px:
                try:
            ...
            if resize_to_target and target_w_px and target_h_px and img.size != (target_w_px, target_h_px):
...
def create_output_folder(base: str) -> str:
    ...
    name = f"Наборы54 макеты от {today}"
...
def build_theme_index(root: str) -> dict[str, str]:
    """
    Индекс папок строится один раз:
    имя папки -> путь.
    Прямые подпапки root имеют приоритет, как в исходной логике.
    """
    index: dict[str, str] = {}
...
def collect_images_strict_order(
    ...
    """
    Сохраняет исходную логику порядка:
    - Блокнот: попарная перестановка тем + дубль фото.
    - Наклейки Love is / Фото наклейки: порядок блоков 3-2-1 внутри троек.
    - Остальное: как в списке.
    """
    blocks: list[dict] = []
    ...
    for row_id, theme in enumerate(theme_names):
        theme = theme.strip()
        ...
        blocks.append({"row_id": row_id, "theme": theme, "files": files})

    # Строго сохраняем порядок тем, как они введены в список.
    # Старые перестановки для Блокнотов и Наклеек отключены.
    ordered_blocks: list[dict] = blocks

    result_files: list[str] = []
    result_row_ids: list[int] = []
    ...
    for block in ordered_blocks:
        ...
        if subfolder == "Блокнот" and notepad_dup and files:
            files_for_theme = [files[0], files[0]]
        else:
            files_for_theme = files

        for i, file_path in enumerate(files_for_theme):
            result_files.append(file_path)
            result_row_ids.append(row_id)
    ...
    return result_files, result_row_ids, result_themes, result_last_flags, align_top, blocks


def draw_img_to_pdf(c, img_path: str, x, y, width, height, err_fn=None, image_cache=None, resize_to_target: bool = True):
    try:
        img_reader = prepare_image_reader(
            img_path,
            width,
            height,
            cache=image_cache,
        ...
        if err_fn:
            err_fn(f"Ошибка вставки {img_path}: {e}")
...
def process_job(
    ...
    """Одна PDF-задача. Координаты вставки оставлены как в исходном скрипте."""
    marked_rows: set[int] = set()

    def mark_row_done(row_id: int, theme: str | None = None, with_log: bool = False):
        if row_id in marked_rows:
            return
        marked_rows.add(row_id)
        if with_log and theme and log_fn:
            log_fn(f"✅ {job['name']}: {theme} — выполнена")
        if theme_done_fn:
            theme_done_fn(row_id)

    try:
        if stop_event.is_set():
            return

        log_fn(f'[{job["name"]}] старт...')
        page_w, page_h = job["page_size"]

        img_files, img_row_ids, img_themes, img_last_flags, align_top, blocks = collect_images_strict_order(
        ...
        # Темы без фото тоже считаем обработанными по этой категории,
        # иначе общий прогресс может не дойти до конца.
        for block in blocks:
            if not block["files"]:
                mark_row_done(block["row_id"], block["theme"], with_log=False)

        if not img_files:
            err_fn(f'Папки/фото не найдены для товара: {job["name"]} ({job["subfolder"]})')
        ...
        # Постеры А3 — каждая картинка на отдельной странице, как в исходной программе.
        if job["subfolder"] == "Постеры А3":
            c = None
            out_path = os.path.join(out_folder, job["filename"])

            for img_path, row_id, theme, is_last in zip(img_files, img_row_ids, img_themes, img_last_flags):
                if stop_event.is_set():
                    break
                pause_event.wait()
                ...
                # Для А3 не уменьшаем до 180 dpi, чтобы не менять смысл полноразмерного листа.
                draw_img_to_pdf(c, img_path, 0, 0, iw, ih, err_fn, image_cache, resize_to_target=False)
                c.showPage()

                if is_last:
                    mark_row_done(row_id, theme, with_log=True)
        ...
        # Брелоки — зеркальная вторая страница. Координаты оставлены как в исходнике.
        if job["subfolder"] == "Брелок" and job.get("mirror", False):
            ...
            for page_imgs, page_row_ids, page_themes, page_last_flags in pages:
                if stop_event.is_set():
                    break
                pause_event.wait()
                ...
                for r in range(rows):
                    ...
                    for col in range(c_in_row):
                        x = x0 + col * img_w * mm
                        # Для брелоков меняем только визуальный порядок строк: темы идут сверху-вниз,
                        # слева-направо, как введены в список.
                        y = y0 + (rows - 1 - r) * img_h * mm
                        pos.append((x, y))

                page_items = list(zip(page_imgs, page_row_ids, page_themes, page_last_flags))

                for (img_path, row_id, theme, is_last), (x, y) in zip(page_items, pos):
                    if stop_event.is_set():
                        break
                    pause_event.wait()
                    draw_img_to_pdf(c, img_path, x, y, img_w * mm, img_h * mm, err_fn, image_cache)
                ...
                for (img_path, row_id, theme, is_last), (x, y) in zip(mirrored_items, pos):
                    if stop_event.is_set():
                        break
                    pause_event.wait()
                    draw_img_to_pdf(c, img_path, x, y, img_w * mm, img_h * mm, err_fn, image_cache)
        ...
        # Общий случай. Координаты оставлены как в исходной программе.
        c = canvas.Canvas(os.path.join(out_folder, job["filename"]), pagesize=(page_w * mm, page_h * mm))
        total_slots = per_page
        pages = [
            (
                img_files[i:i + per_page],
                img_row_ids[i:i + per_page],
        ...
        for page_imgs, page_row_ids, page_themes, page_last_flags in pages:
            ...
            if align_top and count < total_slots:
                ...
                page_row_ids = page_row_ids + [None] * missing
            ...
            for r in range(rows):
                ...
                for col in range(c_in_row):
                    x = x0 + col * img_w * mm
                    # Визуальный порядок: сверху-вниз, слева-направо,
                    # строго как введены наименования тем.
                    y = y0 + (rows - 1 - r) * img_h * mm
                    pos.append((x, y))

            for img_path, row_id, theme, is_last, (x, y) in zip(page_imgs, page_row_ids, page_themes, page_last_flags, pos):
                if stop_event.is_set():
                    break
                pause_event.wait()

                draw_img_to_pdf(c, img_path, x, y, img_w * mm, img_h * mm, err_fn, image_cache)
                if is_last and row_id is not None:
                    mark_row_done(row_id, theme, with_log=True)
        ...
        # Если задание завершилось ошибкой или остановкой, не оставляем прогресс зависшим.
        for row_id, theme in enumerate(theme_names):
            if row_id not in marked_rows:
                mark_row_done(row_id, theme, with_log=False)
...
class App:
    def __init__(self, root: tb.Window):
        self.root = root
        self.root.title("Наборы 54 предмета. Макеты для печати")
    ...
    def _build_ui(self):
        ...
        tb.Label(row3, text="Больше 7 потоков почти не дает прироста", foreground="#c9c9c9").pack(side=LEFT)
    ...
    def process_all(self, root_folder: str, theme_names: list[str], threads_count: int):
        ...
        try:
            ...
            row_category_counts = {row_id: 0 for row_id in range(len(theme_names))}
            fully_done_rows: set[int] = set()
            progress_lock = threading.Lock()

            def mark_theme_category_done(row_id: int):
                if row_id is None:
                    return
                with progress_lock:
                    if row_id in fully_done_rows:
                        return
                    if row_id not in row_category_counts:
                        row_category_counts[row_id] = 0
                    row_category_counts[row_id] += 1
                    if row_category_counts[row_id] >= category_total:
                        fully_done_rows.add(row_id)
                        self.done_themes = len(fully_done_rows)
            ...
            # На случай если последние события прогресса еще не успели отрисоваться.
            if not self.stop_event.is_set() and done < total:
                done = total
                self.done_themes = total
