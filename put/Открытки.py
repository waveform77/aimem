...
def get_category_info(theme_name: str) -> CategoryInfo:
    ...
    if norm.startswith("48открытки"):
        return CategoryInfo("cards_48", "Открытки 48шт", 48)

    if norm.startswith("32открытки"):
        return CategoryInfo("cards_32", "Открытки 32шт", 32)

    if norm.startswith("16открытки"):
        return CategoryInfo("cards_16", "Открытки 16шт", 16)

    return CategoryInfo("cards_other", "Открытки", None)
...
class PDFGenerator:
    def __init__(self, threads: int = 4, dpi: int = DEFAULT_DPI, batch_size: int = PREP_BATCH_SIZE):
        self.threads = max(1, int(threads))
        self.dpi = max(120, int(dpi))
        self.batch_size = max(1, int(batch_size))

    @staticmethod
    def _normal_grid_position(pos: int) -> tuple[float, float]:
        x_off = (PAGE_WIDTH - GRID_COLS * PHOTO_WIDTH) / 2
        y_off = (PAGE_HEIGHT - GRID_ROWS * PHOTO_HEIGHT) / 2

        row, col = divmod(pos, GRID_COLS)
        x = col * PHOTO_WIDTH + x_off
        y = PAGE_HEIGHT - (row + 1) * PHOTO_HEIGHT - y_off
    ...
    def _draw_separator_page(
        ...
        for idx in range(rect_count):
            ...
            c.setLineWidth(1.1)
            c.rect(x, y, PHOTO_WIDTH, PHOTO_HEIGHT, stroke=1, fill=0)

            center_x = x + PHOTO_WIDTH / 2

            # Верхняя часть: категория + пак или только пак
            top_center_y = y + PHOTO_HEIGHT * 0.72

            if category_label:
                title_size = self._fit_font_size(c, category_label, PHOTO_WIDTH * 0.90, 25, min_size=11)
                pack_size = self._fit_font_size(c, pack_text, PHOTO_WIDTH * 0.90, 30, min_size=13)
                ...
                pack_size = self._fit_font_size(c, pack_text, PHOTO_WIDTH * 0.90, 32, min_size=15)
                pack_y = top_center_y - pack_size / 3

                draw_underlined_centered(pack_text, center_x, pack_y, pack_size)

            # Нижняя часть: название темы из списка.
            # Сделано крупнее, но с переносом строк, чтобы не вылезало за прямоугольник.
            theme_lines, theme_font_size, theme_line_height = self._fit_wrapped_text(
                c,
                item.name,
                max_width=PHOTO_WIDTH * 0.90,
                max_height=PHOTO_HEIGHT * 0.42,
                start_size=14,
                min_size=7,
            )

            bottom_center_y = y + PHOTO_HEIGHT * 0.23
    ...
    def _draw_layer(
        ...
        prepared = prepare_batch(existing_paths, PHOTO_WIDTH, PHOTO_HEIGHT, self.dpi, self.threads, err_fn=err_fn, stop_event=stop_event)
...
class App:
    def __init__(self, root: tb.Window):
        self.root = root
        self.root.title("Открытки 7х10см. Макеты для печати")
        ...
        # Тумблер белых листов включен по умолчанию
    ...
    def process_generate(
        ...
        try:
            ...
            self._log(
                f"Формат: открытки {photo_width_mm:g}×{photo_height_mm:g} мм, "
                "16 фото на листе, сетка 4×4"
            )
            ...
            out = os.path.join(save_folder, f"Открытки от {date_str}.pdf")

            gen = PDFGenerator(threads=threads, dpi=dpi)
